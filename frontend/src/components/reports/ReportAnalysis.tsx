...
const ReportAnalysis: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      ...
      try {
        ...
        const response = await fetch("http://192.168.55.47:4000/selected-apis", {
          method: "POST",
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch("http://192.168.55.47:4000/upload", {
        method: "POST",
        body: formData,
      });
      ...
      try {
        const loadResponse = await fetch("http://192.168.55.47:4000/load-documents", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.55.47:4000/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          chat: userMessage,
